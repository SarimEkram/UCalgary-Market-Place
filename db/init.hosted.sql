-- Portable seed for HOSTED MySQL (Aiven / Railway / PlanetScale).
-- Generated from db/init.sql. Differences from the docker version:
--   * no CREATE/USE DATABASE  (import into your host's existing database)
--   * no SET GLOBAL / CREATE EVENT (needs SUPER; unsupported on some hosts)
--   * no LOAD_FILE image seeds (uploads still work at runtime)
...
-- [portable] CREATE EVENT removed: enable the event scheduler on your host if you want auto-cleanup of expired codes.
...
-- [portable] Seed images removed: LOAD_FILE is unavailable on hosted MySQL.
-- Uploaded images are stored as blobs at runtime, so this only affects demo thumbnails.
