...
export default function PostCard({
    ...
    const hasReports = Number(numReports) > 0;
    const [date, setDate] = useState(
        reportDate && hasReports ? dayjs(reportDate).fromNow() : ""
    );
    ...
    const callBackDelete = (ok) => {
        ...
        if (ok) {
            // stay on /profile/2 but refresh data
    ...
    return (
        ...
        >
            ...
            <Typography
                ...
                variant="body1"
                color="textSecondary"
                visibility={hasReports ? "visible" : "hidden"}
            >
                {hasReports ? `${numReports} Reported • ${date}` : ""}
            </Typography>
        </Card>
    );
}
